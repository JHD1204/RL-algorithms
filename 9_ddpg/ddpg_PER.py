...
class DDPGAgent:
    def __init__(self, env, actor_kwargs, critic_kwargs,
        ...
        self.replayer = Experience(replayer_capacity, batch_size, alpha=0.6)
    ...
    def learn(self, observation, action, reward, next_observation, done):
        ...
        # 计算TD error作为transition优先度
        observation_action = np.hstack([observation, action])
        next_action = self.actor_target_net.predict(next_observation)
        next_observation_action = np.hstack([next_observation, next_action])
        next_q = self.critic_target_net(next_observation_action)[0]
        target = reward + self.gamma * next_q * (1. - done)
        TD_error = abs(self.critic_evaluate_net(observation_action) - target)

        self.replayer.add([observation, action, reward, next_observation, done], TD_error)
        ...
        if self.replayer.count >= self.replayer_initial_transitions:
            ...
            for batch in range(self.batches):
                ...
                # TD errors
                # TD_errors = self.critic_evaluate_net(observation_actions) - targets
